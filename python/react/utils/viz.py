...
#                 text=f"{node.name} {node.global_instance_id}",
...
#
#
# def draw_set_geometry(
#     dsg_id: int,
#     node_geometries: Dict[int, Dict],
#     instance_set: InstanceSet,
#     set_label_z: float = 5,
# ) -> Dict:
#     color = get_class_color(instance_set.class_id)
#     nodes_pos = []
#     set_geometries = {}
#     inst_ids_in_dsg = []
#     for inst_id, pos in instance_set.position_histories.items():
#         if dsg_id in pos.keys():
#             nodes_pos.append(pos[dsg_id])
...
#         text_mesh = draw_text_mesh(
#             text=f"Set {instance_set.name} {instance_set.set_id}", position=set_center
#         )
...
# ) -> List:
#     hydra_nodes = map_updater.object_nodes[dsg_id]
#     instance_sets = map_updater.instance_sets
#     mesh.translate(np.array([0, 0, dsg_offset_z]))
#     dsg_viz = []
#     if include_scene_mesh:
#         dsg_viz.append(mesh)
#     node_geometries = {}
#     for id, node in hydra_nodes.items():
...
#     if draw_set:
#         for inst_set in instance_sets.values():
#             set_geometry = draw_set_geometry(
#                 dsg_id=dsg_id,
#                 node_geometries=node_geometries,
#                 instance_set=inst_set,
...
#     draw_text=False,
# ) -> List:
#     hydra_nodes = map_updater.object_nodes[dsg_id_new]
...
#         node_geometries[id] = node_viz
#         dsg_viz += list(node_viz.values())
#     for iset in map_updater.instance_sets.values():
#         for inst_id, ph in iset.position_histories.items():
#             if dsg_id_new in ph.keys() and dsg_id_old in ph.keys():
#                 old_pos = deepcopy(ph[dsg_id_old])
#                 new_pos = deepcopy(ph[dsg_id_new])
